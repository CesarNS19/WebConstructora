...
function cargarModulosFromPermissionOnMenu() {
  $.ajax({
      url: "/SubModule/ListModuleOnProfile",
      type: "POST",
      success: function (res) {
          if (res.ok) {
              console.table(res.data);
              const modulosFromPermission = res.data;
              let groupedModules = {};

              if (Array.isArray(modulosFromPermission)) {
                  modulosFromPermission.forEach(modulo => {
                    console.log(modulo.perStatus);
                      if (modulo.perStatus === 'Activo') {
                          if (!groupedModules[modulo.moDescription]) {
                              groupedModules[modulo.moDescription] = [];
                          }
                          groupedModules[modulo.moDescription].push(modulo);
                      }
                  });
                  ...
                  for (const [moDescription, modules] of Object.entries(groupedModules)) {
                    const moDescriptionLower= moDescription.toLowerCase().replace(/\s+/g, '');
                      let menuItemHTML = `
                          <li class="nav-item">
                              <a class="nav-link dropdown-indicator" href="#${moDescriptionLower}nav" role="button" data-bs-toggle="collapse" aria-expanded="false" aria-controls="${moDescription.replace(/\s+/g, '')}">
                                  <div class="d-flex align-items-center">
                                      <span class="nav-link-icon"><span class="${modules[0].moIcon}"></span></span>
                                      <span class="nav-link-text ps-1">${moDescription}</span>
                                  </div>
                              </a>
                              <ul class="nav collapse" id="${moDescriptionLower}nav">`;

                      modules.forEach(modulo => {
                          menuItemHTML += `
                              <li class="nav-item">
                                  <a class="nav-link drop" href="/${modulo.moController}/${modulo.subAction}">
                                      <div class="d-flex align-items-center">
                                          <span class="nav-link-text ps-1">${modulo.subDescription}</span>
                                      </div>
                                  </a>
                              </li>`;
                      });
                      ...
                      MenuDiv.innerHTML += menuItemHTML;
                  }
              }
              const currentUrl = window.location.pathname;
              console.log(currentUrl);
                const navLinks = document.querySelectorAll('.drop');
                ...
                navLinks.forEach(link => {
                  ...
                  if (link.getAttribute("href") === currentUrl) {
                    ...
                    if (collapse) {
                      ...
                      const toggle = document.querySelector(`a[href="#${collapse.id}"]`);
                      if (toggle) {
